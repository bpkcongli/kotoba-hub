...
- Design system dibuat dulu sebelum feature coding: Tailwind CSS + shadcn/ui + custom token layer. Brand direction: clean study-product, academic but friendly; font pair `Plus Jakarta Sans` + `Noto Sans JP`; token warna final diturunkan dari `Seifuku Navy`, `Slate Blue`, dan `Coral Energy`, lalu dilengkapi neutral surface/text roles serta semantic/support states seperti success dan warning; token radius, spacing, elevation, focus ring, and semantic states didefinisikan sebagai source of truth.
...
- Setiap lesson diperlakukan sebagai loop belajar kecil: learner membaca `lesson.contentBlocks` lebih dulu, lalu wajib mengerjakan `post-study quiz` deterministik berisi tepat `1` pertanyaan `SLOT_FILL`. Pertanyaan diambil dari bank soal lesson berisi tepat `10` kemungkinan soal dengan difficulty `1-10`.
...
- Random Questions di practice hub dibuat sebagai 5-question session by default, dengan komposisi `60% weak skills`, `30% reinforcement`, `10% stretch`. Lesson `post-study quiz` tidak dibuat sebagai `practice_session`; quiz ini memilih tepat `1` pertanyaan dari bank kurasi tetap `10` pertanyaan `SLOT_FILL` yang spesifik untuk lesson tersebut, berdasarkan level pemahaman user saat ini.
- Jangan menyerahkan semua evaluasi ke AI. Untuk kana, vocab match, multiple choice, dan sentence slot-fill yang deterministik, nilai dengan rules biasa. AI hanya dipakai untuk generate candidate set terstruktur saat memang diperlukan, grade free-response, normalize personalization input, dan memberi feedback singkat. Post-study quiz lesson tidak di-generate oleh AI; pertanyaannya harus berasal dari bank soal kurasi deterministik berbasis lesson.
...
- Completion lesson pada MVP diturunkan dari flow belajar, bukan sekadar kunjungan halaman: lesson baru dianggap selesai setelah learner menjawab benar minimal satu `post-study quiz` wajib sehingga `lesson_understanding_snapshots.current_understanding_level` naik dari `0` ke `1`.
- Pemahaman user per lesson dicatat sebagai ladder `0-10` pada `lesson_understanding_snapshots`. Nilai awal adalah `0`; setiap jawaban benar pada post-study quiz atau review lesson menaikkan level maksimal satu langkah sampai `10`; jawaban salah tidak menaikkan level. Level ini menjadi salah satu input personalization dan AI random practice generation, bersama learner profile, weak skills, flashcard result, dan practice history.
...
- Public API contracts yang perlu dikunci: `GET /api/syllabus`, `GET /api/syllabus/units/:slug`, `GET /api/flashcards/decks`, `POST /api/flashcards/sessions`, `POST /api/flashcards/sessions/:id/answer`, `POST /api/practice/sessions/generate`, `POST /api/practice/sessions/:id/answer`, `POST /api/practice/lesson-post-study/next`, `POST /api/practice/lesson-post-study/answer`, `GET /api/progress/overview`, `GET /api/progress/timeline`, `POST /api/personalization/assessment`.
...
6. Implement AI practice module dengan provider abstraction, prompt templates, structured outputs, grading pipeline untuk random practice, direct deterministic lesson post-study quiz, and recommendation logic tied ke skill mastery plus lesson understanding level.
